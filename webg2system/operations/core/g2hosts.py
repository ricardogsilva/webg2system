    ...
    def _match_file_patterns(self, **kwargs):
        ...
        for file_setting in eval(query_set):
            for pattern_string in file_setting.filepattern_set.all():
                reg_exps.append(pattern_string.string.replace('#', '.*'))
        return reg_exps

    def _get_deletable_regexps(self):
        '''
        ...
        return self._match_file_patterns(toDelete=True)

    def _is_deletable(self, path, deletable_regexps=None):
        '''
        ...
        Inputs:
            ...
            deletable_regexps - A list of strings to be interpreted
                ...
                file patterns that are allowed to be deleted. If
                None (the default) a new list is generated by
                querying the relevant file settings from the
                systemsettings django app.
...
class G2LocalHost(G2Host):
    ...
    def _delete_old_files(self, older_than):
        ...
        have their 'toDelete' attribute set to True.
        '''
